...
export const postFood = async(req, res) => {
  const { foodName, price, ingredients, category,rate } = req.body;
 ...
 try{
  ...
  // await Food.insertMany(data);
    ...
    const fooddata = new Food({foodName : foodName, description: ingredients, price : price, rate: rate , category: category})
    await fooddata.save()
    res.status(201).json({
